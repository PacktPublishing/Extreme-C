// File name: ExtremeC_examples_chapter8_5.c
// Description: A classic example which demonstrates how general
//              semaphores can be used to create molecules of water
//              using threads of hydrogen and oxygen atoms. Each
//              oxygen thread should wait for two hydrogen threads
...
#include <semaphore.h> // Semaphores are not exposed through pthread.h
...
int main(int argc, char** argv) {
  ...
  for (int i = 0; i < 150; i++) {
    if (pthread_join(thread[i], NULL)) {
      printf("The thread could not be joined: %s\n", strerror(errno));
      exit(3);
    }
  }
